...
class ScheduleErrandInput(BaseModel):
    task_reference: str
    scheduled_for: str = Field(description="ISO-8601 time with timezone")
...
class LiveLocationAgent:
    ...
    def facts(self, chat_id: int) -> ToolResult:
        ...
        return ToolResult(
            ok=True,
            data={
                "known": bool(user and user.get("last_lat") is not None),
                "area": user.get("last_area") if user else None,
                "updated_at": user.get("last_seen_at") if user else None,
            },
        )
...
class CalendarAgent:
    async def facts(self, user: dict) -> ToolResult:
        calendar = CalendarFacts.model_validate(services.calendar_snapshot(user))
        tasks = []
        for t in db.open_tasks(user["chat_id"]):
            trip = None
            if user.get("last_lat") is not None:
                trip = (
                    eta_minutes(
                        haversine(
                            user["last_lat"],
...
class OrchestrationAgent:
    def __init__(self, create_from_parsed, complete_by_reference, schedule_task, make_context):
        self.create_from_parsed = create_from_parsed
        self.complete_by_reference = complete_by_reference
    ...
    async def respond(self, user: dict, text: str) -> AgentReply:
        if not OPENROUTER_API_KEY:
            return AgentReply(
                text="I need an OpenRouter key to help with errands and scheduling."
            )
        prompt = f"""
        ## Introduction
        ...
        return AgentReply(
            text=await _run_agent(
                prompt, self._tools(user), "I couldn't process that just now."
            ),
            result=self.action,
            action_kind=getattr(self, "action_kind", None),
        )
    ...
    def _tools(self, user: dict) -> list[AgentTool]:
        ...
        def create(payload: CreateErrandInput) -> ToolResult:
            result = owner.create_from_parsed(
                user, {"intent": "create", **payload.model_dump()}
            )
            owner.action = result
            owner.action_kind = "create"
            return ToolResult(
                ok=not hasattr(result, "message"),
                data={"task": result if isinstance(result, dict) else {}},
                error=getattr(result, "message", None),
            )

        def complete(payload: CompleteErrandInput) -> ToolResult:
            result = owner.complete_by_reference(user["chat_id"], payload.task_reference)
            owner.action = result
            owner.action_kind = "complete"
            return ToolResult(
                ok=not hasattr(result, "message"),
                data={"task": getattr(result, "task", {})},
                error=getattr(result, "message", None),
            )

        def schedule(payload: ScheduleErrandInput) -> ToolResult:
            result = owner.schedule_task(
                user["chat_id"], payload.task_reference, payload.scheduled_for
            )
            owner.action = result
            owner.action_kind = "schedule"
            return ToolResult(
                ok=not hasattr(result, "message"),
                data={"task": result if isinstance(result, dict) else {}},
                error=getattr(result, "message", None),
        ...
        async def calendar(_: Empty) -> ToolResult:
            return await owner.calendar.facts(user)

        def location(_: Empty) -> ToolResult:
            return owner.live_location.facts(user["chat_id"])

        return [
            AgentTool("create_errand", "Create an errand.", CreateErrandInput, create),
            AgentTool(
                "complete_errand", "Complete an errand.", CompleteErrandInput, complete
            ),
            AgentTool(
                "schedule_errand",
                "Create or edit an errand schedule.",
                ScheduleErrandInput,
                schedule,
            ),
            AgentTool("calendar_facts", "Read current/upcoming calendar JSON.", Empty, calendar),
            AgentTool("location_facts", "Read persisted location JSON.", Empty, location),
        ]
    ...
    try:
        ...
        for _ in range(4):
            ...
            if not message.tool_calls:
                return (message.content or fallback).strip()
            messages.append(
                {
                    "role": "assistant",
            ...
            for call in message.tool_calls:
                try:
                    ...
                    value = tool.execute(
                        tool.input_model.model_validate_json(call.function.arguments)
                    )
        ...
        final = await client.chat.completions.create(
            model=OPENROUTER_MODEL, temperature=0, messages=messages
        )
        return (final.choices[0].message.content or fallback).strip()
    except Exception as exc:
        log.warning("agent loop failed: %s", exc)
        return fallback
